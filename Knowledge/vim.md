...
之前有个做vim的教程：[[Study Log/vimtutor|vimtutor]]，还是不够全。这次在github上找了个全的：[chloneda/vim-cheatsheet: 🍁Vim cheat sheet with everything you want to know.](https://github.com/chloneda/vim-cheatsheet)

我把它down到这里，然后再补充上我要的东西。

## 我常用的

把我常用的快捷键总结下。

1. `dj`: 等价于`d2d`，`dk`是删除上面两行
2. `ci(`：比如你`if(xxx)`，就可以替换括号里的东西，还有`<`，`{`，`[`等都可以用
3. `<ctrl>u`和`<ctrl>d`是我常用的滚动方式，是滚半屏。相对的，`<ctrl>y`和`<ctrl>e`是我常用的微调操作，这两个不会东光标，只会把屏幕上下挪一行。这四个导航是配合着用的。
4. `e`是到下一个词的结尾，相反的操作是`ge`，到上一个词的词尾。
5. 用`D`代替`d$`，用`C`代替`c$`。
6. 对于第2条，小括号可以用`b`表示，大括号可以用`B`表示。比如`di(`可以替换成`dib`，快很多。
