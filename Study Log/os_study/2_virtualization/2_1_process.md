...
对于zombie，需要特别强调。首先是这里的清理，不是代表这个进程的内存。僵尸态已经不占内存了，因为没有意义。**它占的是进程列表中的一项**；其次是什么时候才是僵尸态。比如线程，主线程调用join来等待它派生出来的线程结束。进程也是一样的，父进程会调用waitXXX来等待子进程结束。而如果父进程忘了调用wait，子进程就会一直保持僵尸状态。

- [ ] #TODO tasktodo1731343144344 给一个僵尸态进程的例子。 ➕ 2024-11-12 ⏫ 🆔 595p95 
