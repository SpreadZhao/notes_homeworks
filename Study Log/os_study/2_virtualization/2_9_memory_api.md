...
上面的代码中，两种内存的分配都出现了。堆内存是我们调用malloc申请的。同时，这里也有栈内存的分配。就是分配给x这个指针了。这里x保存的，其实就是malloc函数的返回值，它是`int *`类型的。
...
上面的malloc和free都**不是**系统调用，只是标准库里的函数而已。因此，malloc library管理的是你的虚拟地址空间，但是这个library是构建在某些系统调用之上的。这些系统调用负责向OS申请一些内存，还有把一些内存交换给操作系统。

这些系统调用的其中之一是brk，这个系统调用的作用是：改变一个程序的**break**的位置。break是什么呢？break指的是**堆内存的尾部**。所以，brk做的事情就是：接收一个参数，这个参数就是新的break的地址，这样根据新地址和老地址比较，就知道我是想扩内存还是缩内存。

还有一个sbrk，传的是相对值。

注意，你应该永远也别直接调用brk或者sbrk。要用封装好的malloc和free。

最后，你也可以用mmap系统调用来向操作系统申请内存。如果你传了适当的参数(`MAP_ANONYMOUS`)，mmap会在你的进程中给你创建一个匿名的内存——和任何文件都没关系，而是映射你磁盘上的交换空间（swap space），这样这些内存就可以像堆内存那样使用了。

### 2.9.6 Other Calls

calloc：分配内存，并自动设置成0。这里主要是为了避免上面的错误2：忘记初始化内存。

realloc：这个可以看man去了解。主要功能就是修改分配内存的大小。如果变大，那么原来的内容都不会丢失；如果变小，那么内容会按照新的大小去截断。如果传入的大小是0，那么相当于free。
